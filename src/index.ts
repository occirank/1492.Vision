...
// Create MCP server
const server = new McpServer({ name: "1492.Vision MCP", version: "1.0.0" });
...
app.get("/sse", async (req: Request, res: Response) => {
  try {
    // Read access_token header directly 
    const apiKey =
    ...
    if (!apiKey) {
      return res.status(400).send("Missing access_token header");
    }
...
app.get("/health", (_req, res) => {
  res.status(200).send({ status: "ok", server: "1492.Vision MCP Server", version: "1.0.0" });
});

// Start server
app.listen(PORT, () => {
  console.log(`1492.Vision MCP Server running on http://localhost:${PORT}`);
